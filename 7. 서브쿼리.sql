...
-- # 단일행 서브쿼리
--  서브쿼리의 조회 결과가 1건 이하인 경우 -> 0건 나와도 단일행 서브쿼리
...
;  -- single-row subquery returns more than one row 오류남 -> where절의 서브쿼리다 단일행이 아님 
...
-- # 다중행 서브쿼리
-- 서브쿼리의 조회 건수가 0건 이상인 것 -> 단일행 나오면 다중행에 포함
-- ## 다중행 연산자
...
JOIN tb_emp_certi B
ON A.emp_no = B.emp_no
WHERE B.certi_cd IN ( --다중행 서브쿼리이므로 IN 사용
                        SELECT certi_cd 
                        FROM tb_certi 
                        WHERE issue_insti_nm = '한국데이터베이스진흥원'
                    )
...
WHERE (A.dept_cd, A.birth_de) IN (
                        ...
                        FROM tb_emp
                        GROUP BY dept_cd
                        HAVING COUNT(*) >= 2 --dept_cd의 인원이 2명 이상                        
                    )
ORDER BY A.emp_no
;
