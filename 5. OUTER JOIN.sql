...
-- INNER JOIN 
SELECT E.emp_no, E.emp_nm, E.dept_cd 
...
--매칭 안되면 오른쪾 애들 NULL로 나옴
-- ANSI 표준
SELECT 
    A.emp_no, A.emp_nm, B.dept_cd, B.dept_nm
FROM tb_emp A
LEFT OUTER JOIN tb_dept B --tb_emp는 매칭안되도 다 보여줘. 만약 매칭되면 부서 정보도 같이 보여주세요
ON A.dept_cd = B.dept_cd
ORDER BY A.emp_no 
;
-- 댓글이 없는 개시물, 파일 첨부 안된 애들도 조회 -> LEFT OUTER JOIN
...
WHERE A.dept_cd = B.dept_cd (+) --left JOIN
-- WHERE A.dept_cd(+)  = B.dept_cd --right join
ORDER BY A.emp_no
;
...
-- #FULL OUTER JOIN (왼쪽/오른쪽 다보여줘, 매칭되면 다 보여주고, 매칭안되면 null)
