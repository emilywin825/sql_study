...
SELECT 
    emp_nm EN, birth_de BD --위에 있는 별칭 이용
FROM tb_emp
WHERE birth_de BETWEEN '19600101' AND '19691231'
UNION
SELECT 
...
START WITH A.direct_manager_emp_no IS NULL -- 어디서부터 시작할지 정함.
CONNECT BY PRIOR A.emp_no = A.direct_manager_emp_no -- 순방향 전개/역방향 전개를 정함
--PRIOR이 자식앞에 붙으면 순방향 전개, 부모앞에 붙으면 역방향 전개
ORDER SIBLINGS BY A.emp_no DESC -- 같은 SIBLINGS끼리는 emp_no로 내림차 정렬
;
